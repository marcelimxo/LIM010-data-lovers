...
describe('loginValidate', () => {
  ...
  it('debería retornar "ok" si la contraseña y el usuario son iguales a "LABORATORIA"', () => {
    expect(app.loginValidate('LABORATORIA', 'LABORATORIA')).toBe('ok');
  });
  it('debería retornar "Usuario o contraseña incorrectas" si la contraseña o el usuario son diferentes a "LABORATORIA"', () => {
    expect(app.loginValidate('USUARIO', 'LABORATORIA')).toBe('Usuario o contraseña incorrectas');
  });
  it('debería retornar "Ingrese el usuario" si el usuario se encuentra vacio', () => {
    expect(app.loginValidate('', 'LABORATORIA')).toBe('Ingrese el usuario');
  });
  it('debería retornar "Ingrese la contraseña" si la contraseña se encuentra vacio', () => {
    expect(app.loginValidate('LABORATORIA', '')).toBe('Ingrese la contraseña');
  });
});

describe('dataCurated', () => {
  it('debería ser una función', () => {
    expect(typeof app.dataCurated).toBe('function');
  });

  it('debería devolver un nuevo objeto', () => {
    expect(typeof app.dataCurated(LOL.data, app.obj)).toBe('object');
  });
  it('debería retornar la data curada para "LeeSin" ', () => {
    expect(app.dataCurated(LOL.data, app.obj)[59].splash).toEqual('img/splash/LeeSin_Splash_11.jpg');
  });
});
...
describe('selectedData', () => {
  ...
  it('debería retornar un nuevo objeto con atributos personalizados', () => {
    ...
    expect(app.selectedData(app.dataCurated(LOL.data, app.obj))[0]).toEqual(selectedDataSalida);
  });
});
